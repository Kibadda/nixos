    ...
    home-manager.users.${meta.username} = {
        ...
        settings = {
          top = {
            ...
            modules-right = concatLists [
              (optional (cfg.waybar.battery) "battery")
              (optional (cfg.waybar.backlight) "backlight")
      ...
      home = {
        packages = concatLists [
          (optional (cfg.bluelight) pkgs.hyprshade)
          [
      ...
      wayland.windowManager.hyprland = {
        ...
        settings = {
          ...
          exec-once = concatLists [
            (optional (cfg.wallpaper != null) "${pkgs.hyprpaper}/bin/hyprpaper")
            (optional (cfg.waybar.enable) "${pkgs.waybar}/bin/waybar")
            (optional (cfg.cursor != null) "hyprctl setcursor \"${cfg.cursor.name}\" ${toString cfg.cursor.size}")
          ];

          exec = concatLists [
            (optional (cfg.bluelight) "${pkgs.hyprshade}/bin/hyprshade auto")
          ];
        } // cfg.settings;
      };
